...
3. Run the checks and validate the proposed tag locally (this does not create a tag):
   ...
   python3 .github/release.py validate --tag v0.2.0
...
5. Tag the reviewed commit and push that tag:
   ...
   ```bash
   git tag -a v0.2.0 -m "Urbilateria 0.2.0"
...
| --- | --- | --- |
| `urb-v0.2.0-x86_64-unknown-linux-gnu.tar.gz` | Ubuntu 22.04 | Linux x86_64 with glibc 2.35+ |
| `urb-v0.2.0-aarch64-apple-darwin.tar.gz` | macOS 15, Apple Silicon | Apple Silicon macOS 13+ |
...
```bash
cargo build --release --locked --features ui --bin urb
python3 .github/release.py package --tag v0.2.0 \
